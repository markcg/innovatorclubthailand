...
class cURL {
    ...
    public function get_to_server($url, $data = array()) {
        $frinstar = new Frinstar();
        $data_w_key = array_merge($data, $frinstar->Key());
        $this->setURL($url, $data_w_key);
    ...
    public function patch($url, $data = array()) {
        ...
        $this->setOpt(CURLOPT_POSTFIELDS, $data_w_key);
        return $this->exec();
    }
    ...
    public function post_to_server($url, $data = array()) {
        if (is_array($data) && empty($data)) {
            $this->unsetHeader('Content-Length');
        }
        $frinstar = new Frinstar();
        ...
        $this->setOpt(CURLOPT_CUSTOMREQUEST, 'POST');
        $this->setOpt(CURLOPT_POST, true);
        $this->setOpt(CURLOPT_POSTFIELDS, $this->postfields($data_w_key));
        return $this->exec();
    }
